...
class Polynomial:
    ...
    def __mod__(self, other):
        ...
        ratio = 0
        if self[0] != 0 and other[0] != 0:  # Check for zero values
        ...
        while len(num) >= len(other):
            ratio = 0
            if num[0] != 0 and other[0] != 0:  # Check for zero values
                ratio = glog(num[0]) - glog(other[0])

            if ratio == 0:
                num = num[1:]
                continue

            mult = [gexp(glog(x) + ratio) for x in other]
            num = [x ^ y for x, y in zip(num, mult * len(num))]
